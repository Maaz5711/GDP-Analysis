...
# --- Constants (Copied from dashboard.py) ---
valid_regions = [
...
NON_COUNTRY_NAMES = {
    ...
    "European Union", "High income", "IBRD only", "IDA & IBRD total", "IDA blend", "IDA only",
    ...
    "Low income", "Lower middle income", "Middle East, North Africa, Afghanistan & Pakistan",
    ...
    "Upper middle income", "West Bank and Gaza",
}

# --- Plotting Functions (Adapted for saving) ---

def plot_regional_histogram(ax, data, region, year):
    # Logic copied exactly from your dashboard
...
def plot_international_pie(ax, data, year):
    ...
    wedges = ax.pie(region_values, colors=colors, startangle=140, autopct="%1.1f%%")[0]
    ax.set_title("Regional GDP Distribution (" + str(year) + ")")
    ax.legend(wedges, region_names, loc="upper center", bbox_to_anchor=(0.5, -0.05), ncol=3, fontsize=7)

def plot_international_bar(ax, data, year):
    year_data = processor.filter_by_year(data, year)
    year_data = list(filter(lambda row: row["Country"] not in NON_COUNTRY_NAMES, year_data))
    if len(year_data) == 0: return
...
# --- Main Execution for Saving ---
if __name__ == "__main__":
    # 1. Setup paths
    # Assuming data is in ../data/gdp.csv relative to src, or just data.csv if in same folder
    # We will assume standard structure: data/gdp.csv exists in repo root
    data_path = "data/gdp.csv" 
    if not os.path.exists(data_path):
        data_path = "../data/gdp.csv" # Fallback check
    ...
    fig.suptitle(f"International GDP Analysis ({year})", fontsize=16)
    
    # SAVE instead of SHOW
    save_path = os.path.join(output_dir, "International_Report.png")
    fig.savefig(save_path)
    print(f"Saved: {save_path}")
    tools.close(fig) # Close memory to prevent empty files
